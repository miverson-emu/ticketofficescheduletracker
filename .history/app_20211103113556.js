...
(req, res) => {
	...
	existingId = if(events.find(event => event.id == newEvent.id)) {
		errors.find((error) => error.title == "title").error ==
	}
	existingTitle = events.find(event => event.title == newEvent.title)
