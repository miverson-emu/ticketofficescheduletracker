...
(req, res) => {
	...
	fs.writeFile(fileName, toJSON(fileContents), (error) => {
		res.send((error) ? errorMessage : null);
	})
})
...
app.post("/remove", (req, res) => {
	...
	getJSON(db).then((elements) => {

		where = elements.findIndex(worker => worker[key] == id)
		elements.splice(where)
		return elements
	})
	.then ((elements) => {
		res.send(write(elements, file, error))
	})
			...
			.then((data) => {
				if(data.errors.length > 0) {
					...
					$.post("../w", 
					{
						...
						"fileContents": data.workers, 
						"errorMessage": "There was a problem adding this worker. Try again later."
					}, 
					(write) => {
						res.send(write)
					})
...
(req, res) => {
	...
	getJSON("data/workers.json").then((workerData) => {
	...
	.then((modifiedWorkerData) => {
		$.post("../w", 
		{
			"fileContents": modifiedWorkerData, 
			"fileName": "data/workers.json", 
			"errorMessage": error
		}, 
		(write) => {
...
(req, res) => {
	...
	getJSON("data/workers.json").then((workers) => {
		...
		.then((modifiedWorkerData) => {
			res.send(write(modifiedWorkerData, "data/workers.json", "There was a problem updating this user."))
		})
