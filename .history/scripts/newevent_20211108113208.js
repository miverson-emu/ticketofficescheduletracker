function formatIDNumber(input, length) {
	if(input.value.length < length && input.value < 10){
				...
				errors: [
					{
						"title": "title", 
						"error": false,
						"error-id": "title-exists-error"
					}, 
					{
						"title": "id", 
						"error": false, 
						"error-id": "id-exists-error"
					}
				]
			}, 
			(res) => {
				if(typeof res == "object"){
					...
					if(res[key].error) {
						document.getElementById(res[key]["error-id"]).style.display = "block"
					}
				})
				}	
				else{
