		...
		.then((event) => {
			if(event) {
				...
				.then(() => {
					...
					[...document.getElementsByName("workHours-start")][0].value = convertTimeFormat(start);
					[...document.getElementsByName("workHours-end")][0].value = convertTimeFormat(end);

					var [start, end] = event["hours"].split(" - ");
					[...document.getElementsByName("hours-start")][0].value = convertTimeFormat(start);
					[...document.getElementsByName("hours-end")][0].value = convertTimeFormat(end);

					Object.keys(event).forEach((key) => {
						try{
							if(!["category", "subcategory", "date", "id", "workHours", "hours"].includes(key)){
								console.log(key);
