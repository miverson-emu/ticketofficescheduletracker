		...
		.then((event) => {
			if(event) {
				...
				.then(() => {
					...
					addSubcategoryOptions(event["category"])
				})
				.then(() => {
					//set subcat
				...
				.then(() => {
					...
					//set id
					// [...document.getElementsByName("date")][0].value = event["id"][1:]
					console.log(event["date"].substring(2))

					Object.keys(event).forEach((key) => {
						try{
							if(!["category", "subcategory", "date"].includes(key)){
...
function addSubcategoryOptions(selectedCategory){
	getJSON("data/util.json").then((resources) => {
		eventCategories = resources.eventCategories;
