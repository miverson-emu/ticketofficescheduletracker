...
function completedForm(){
	...
	inputs.map((input) => {
		if (input.value == "") {
			...
			try{
				document.getElementById(input.name + "-error").style.display = "inline"
			}
			catch(e) {
...
function addWorker(){
	newWorker = ifgetWorkerModificationFormData();
	...
	if(add) {
		...
		.then((workers) => {
			return write(workers, "data/workers.json", "There was a problem adding this worker.")
		})
	}
}
